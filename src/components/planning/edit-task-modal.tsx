    ...
    if (data.dailyTasksString && data.dailyTasksString.trim() !== "") {
      try {
        ...
        if (Array.isArray(newDailyTasksRaw)) {
          parsedDailyTasks = newDailyTasksRaw.map((dt: DailyTaskRaw, dtIndex: number) => ({
            id: `daily-${Date.now()}-${dtIndex}`,
            dayDescription: dt.dayDescription,
            status: 'todo',
            subTasks: Array.isArray(dt.subTasks) ? dt.subTasks.map((st: { description: string; status: string }, stIndex: number) => ({
              id: `sub-${Date.now()}-${dtIndex}-${stIndex}`,
