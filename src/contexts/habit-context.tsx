...
export function HabitProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      ...
      if (savedHabitsJson) {
        try {
          const parsedHabits = JSON.parse(savedHabitsJson).map((h: HabitRaw) => ({
            ...h,
  ...
  const markHabitDone = useCallback((habitId: string) => {
    setHabits(currentHabits => {
      const updatedHabits = currentHabits.map(habit => {
        if (habit.id === habitId) {
          const now = new Date();
          const lastCompletedDate = habit.lastCompletedDate?.toDateString();
          
          if (lastCompletedDate === now.toDateString()) {
            // Already completed today, increment count
            return {
              ...habit,
              completedToday: Math.min(habit.completedToday + 1, habit.goal),
            };
          } else {
            // First completion today, reset count and update streak
            const newStreak = lastCompletedDate === new Date(now.getTime() - 24 * 60 * 60 * 1000).toDateString() 
              ? habit.streak + 1 
              : 1;
